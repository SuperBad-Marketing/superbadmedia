...
- **Backups:** Litestream sidecar process streams every write to a Cloudflare R2 bucket in near-real-time. Recovery point objective (RPO) measured in seconds, not hours. R2 has no egress fees — ideal for backup use cases. Config: `litestream.yml` at project root. Full restore procedure: `docs/dr-runbook.md`.
- **ATO 7-year retention (non-negotiable):** Australian Tax Office requires financial records (invoices, payments, subscriptions) to be retained for 7 years. The Cloudflare R2 bucket used for Litestream backups must have its object lifecycle policy set to **never expire** objects. No delete-after rules. No transition rules that would remove old backups. This is configured once in the Cloudflare dashboard — see `docs/dr-runbook.md §"ATO 7-year retention"`. In v1, no delete tooling is shipped for invoices, deals, subscriptions, or payments. Soft-delete flags (if needed) do not physically remove records.
- **Full-text search:** SQLite FTS5 extension (built in). Used for inbox, prospect, and client search. Zero extra infrastructure.
- **Mandatory Phase 5 verification:** before Lite processes any live billing event, an explicit session must nuke the running container, restore SQLite from R2 into a scratch environment, and verify data integrity. This session is a precondition for launch. See `docs/dr-runbook.md §"Restore drill"` and `LAUNCH_READY.md`.
