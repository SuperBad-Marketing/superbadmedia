...
9. **Content Engine cross-platform syndication.** Auto-publish blog content to external platforms (Medium, LinkedIn articles, Ghost, Beehiiv, WordPress.com) for backlink generation and referral traffic. Canonical version stays on superbadmedia.com.au — syndicated copies include `rel=canonical` or "Originally published at" links so SEO ranking accrues to the home domain, not the platform. Each platform is a small adapter off the existing fan-out handler (CE-6). Substack excluded — no public posting API. Added 2026-04-17.
10. **AI search optimisation layer.** A set of enhancements to make superbadmedia.com.au and subscriber blogs more visible to AI search engines (Perplexity, ChatGPT search, Google AI Overviews). Three components:
    - **`llms.txt`** at site root — plain-text file describing the business, services, and key pages for LLM crawlers (analogous to `robots.txt`). Trivial to generate per subscriber from Brand DNA.
    - **FAQ schema markup** on marketing site and subscriber blog pages — structured question-and-answer JSON-LD that AI search engines pull from heavily for direct-answer citations.
    - **Conversion heatmap on landing/funnel/checkout** events (already parked below) feed into which questions are worth targeting.
    Content Engine blog generation prompts already produce JSON-LD Article schema (Q22); the v1.0 prompt tuning note below covers citation-friendly structure. This item adds the `llms.txt` generator and FAQ schema layer on top. Added 2026-04-17.
