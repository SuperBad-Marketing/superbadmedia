...
export function PortalChatTab({
  ...
  const groupedByContact = new Map<string, PortalChatMessageRow[]>();
  for (const msg of chatMessages) {
    const key = msg.contact_id;
    if (!groupedByContact.has(key)) {
      groupedByContact.set(key, []);
    }
    groupedByContact.get(key)!.push(msg);
  }

  const groups = Array.from(groupedByContact.entries()).map(([contactId, msgs]) => ({
  ...
  return (
    <div className="space-y-5 px-4 pb-10">
      ...
      {groups.map(({ contact, contactId, messages }) => (
        <section
          key={contactId}
          aria-label={`Chat with ${contact?.name ?? "Unknown"}`}
