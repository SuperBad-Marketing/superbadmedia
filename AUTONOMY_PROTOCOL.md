...
- **Admin-interior surfaces (added 2026-04-15, mockup landed `admin-polish-0`):** any `UI` session touching a `/lite/admin/**` route *interior* (not just `AdminShell` chrome) must cite `mockup-admin-interior.html` in brief §2a as a binding reference. If the surface introduces a new admin primitive not yet covered by the shared reference (e.g. timeline, thread view, mini-calendar), cut a surface-specific `mockup-admin-<surface>.html` *first*, back-fill the new primitive into `mockup-admin-interior.html`, then cite both in §2a. Admin-interior sessions that cite only brand-guidelines + `/lite/design` tokens will ship generic styling — documented failure mode: Wave 5 SP-3/4/6/9 + Wave 6/7 admin surfaces, caught 2026-04-15 in Andy's post-`admin-chrome-1` walkthrough. G10 parity + G10.5 reviewer fidelity both grade against `mockup-admin-interior.html` for admin-interior work, the same way they grade against per-route mockups for customer-facing surfaces.
- **Portal / Intro Funnel / Cockpit surfaces (added 2026-04-19, PM-6):** any `UI` session building or modifying a net-new customer-facing portal route (`/lite/portal/**`), intro-funnel route (`/get-started/**`), or cockpit route (`/lite/admin/cockpit/**`) must cite the matching mockup in brief §2a: `mockup-client-portal.html` for portal, `mockup-intro-funnel.html` for funnel, `mockup-cockpit-interactive.html` for cockpit. If the session introduces a surface variant not yet covered (e.g. a portal sub-page with no mockup coverage), cut a surface-specific `mockup-<surface>.html` first, then cite both. These surfaces were unbuilt when the visual-references-binding rule landed (2026-04-14) — the same "spec-only → generic styling" failure mode documented for admin-interior applies here. G10 parity + G10.5 reviewer fidelity grade against the cited mockup(s). Downstream sessions affected: CM-5+ (portal), IF-1+ (funnel), DC-1+ (cockpit).
