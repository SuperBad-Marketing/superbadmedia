...
/**
 * PortalRevealClient — the cinematic Brand DNA reveal for portal clients.
 *
 * Same visual/motion treatment as the admin RevealClient (BDA-3), but
 * without NextAuth SessionProvider (portal uses cookie auth, not JWT).
 *
 * Calls `markPortalProfileComplete` instead of admin `markProfileComplete`.
 * No session.update() — the portal gate reads profile status from DB on
 * each request, not from a cached JWT claim.
 *
 * Optionally shows a retake comparison narrative below the portrait when
 * this is a retake (version > 1).
 *
 * Owner: BDA-5.
 */
...
import { markPortalProfileComplete } from "../actions";

interface PortalRevealClientProps {
  profileId: string;
...
export function PortalRevealClient({
  profileId,
  firstImpression,
  prosePortrait,
  sectionInsights,
  ...
  alreadyComplete,
  comparisonNarrative,
}: PortalRevealClientProps) {
  const { play } = useSound();

  const [phase, setPhase] = React.useState<
    "impression" | "sections" | "portrait"
  >("impression");
  ...
  React.useEffect(() => {
    ...
    const fire = window.setTimeout(() => {
      void (async () => {
        await markPortalProfileComplete(profileId);
        if (cancelled) return;
      })();
    }, 800);
    return () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Retake comparison (when this is a retake) */}
        {phase === "portrait" && comparisonNarrative && (
          <motion.section
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ ...houseSpring, duration: 1.2, delay: 0.6 }}
            style={{
              paddingTop: 32,
              borderTop: "1px solid rgba(253, 245, 230, 0.1)",
              display: "flex",
...
function splitImpressionHeadline(text: string): {
  ...
  if (splitMatch) {
    return {
      ...
      headlineAccent: splitMatch[3].trim(),
    };
  }
