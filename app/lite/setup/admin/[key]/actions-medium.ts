"use server";

import { randomUUID, createHash } from "node:crypto";
import { auth } from "@/lib/auth/auth";
import { db } from "@/lib/db";
import { wizard_completions } from "@/lib/db/schema/wizard-completions";
import { registerIntegration } from "@/lib/integrations/registerIntegration";
import { verifyCompletion } from "@/lib/wizards/verify-completion";
import {
  mediumWizard,
  verifyMediumToken,
  type MediumPayload,
} from "@/lib/wizards/defs/medium";
import type { CelebrationCompleteResult } from "@/components/lite/wizard-steps/celebration-step";

export async function testMediumTokenAction(
  token: string,
): Promise<{ ok: true; userId: string } | { ok: false; reason: string }> {
  if (!token) return { ok: false, reason: "Paste the token first." };
  return verifyMediumToken(token);
}

function contractVersion(): string {
  const keys = mediumWizard.completionContract.required
    .map((k) => String(k))
    .sort();
  return createHash("sha256")
    .update(JSON.stringify({ key: mediumWizard.key, required: keys }))
    .digest("hex")
    .slice(0, 16);
}

export async function completeMediumAction(
  payload: MediumPayload,
): Promise<CelebrationCompleteResult> {
  const session = await auth();
  if (!session?.user?.id) {
    return { ok: false, reason: "Session expired, sign in again." };
  }
  const ownerId = session.user.id;
  const ctx = { ownerType: "admin" as const, ownerId };
  const wizardCompletionId = randomUUID();

  try {
    const credentialBlob = JSON.stringify({
      integrationToken: payload.integrationToken,
      userId: payload.userId,
    });

    const { bandsRegistered } = await registerIntegration({
      wizardCompletionId,
      manifest: mediumWizard.vendorManifest!,
      credentials: { plaintext: credentialBlob },
      metadata: { medium_user_id: payload.userId },
      ownerType: "admin",
      ownerId,
    });

    const verified = await verifyCompletion(mediumWizard, payload, ctx);
    if (!verified.ok) {
      return { ok: false, reason: verified.reason };
    }

    await db.insert(wizard_completions).values({
      id: wizardCompletionId,
      wizard_key: mediumWizard.key,
      user_id: ownerId,
      audience: "admin",
      completion_payload: payload,
      contract_version: contractVersion(),
      completed_at_ms: Date.now(),
    });

    return {
      ok: true,
      observatorySummary: bandsRegistered.length
        ? `Bands registered: ${bandsRegistered.join(", ")}.`
        : "Medium connected.",
    };
  } catch (err) {
    return {
      ok: false,
      reason:
        err instanceof Error
          ? err.message
          : "Something went sideways finishing up.",
    };
  }
}
