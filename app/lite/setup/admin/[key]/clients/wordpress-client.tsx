"use client";

import * as React from "react";
import { WizardShell } from "@/components/lite/wizard-shell";
import type {
  WizardStepDefinition,
  WizardAudience,
} from "@/lib/wizards/types";
import { completeWordPressAction } from "../actions-wordpress";
import {
  type WordPressPayload,
  maskWordPressToken,
  maskWordPressSiteId,
  wordpressCredentialsSchema,
} from "@/lib/wizards/defs/wordpress";
import type { CelebrationCompleteResult } from "@/components/lite/wizard-steps/celebration-step";
import { useAdminShell, type StepStates } from "./use-admin-shell";

type PasteCredentialsState = {
  values: { accessToken: string; siteId: string };
};

type ReviewState = {
  summary: { label: string; value: string }[];
  confirmed: boolean;
};

export type WordPressClientProps = {
  audience: WizardAudience;
  steps: WizardStepDefinition[];
  outroCopy: string;
  expiryDays: number;
};

function initialWordPressStates(outroCopy: string): StepStates {
  return {
    "paste-credentials": {
      values: { accessToken: "", siteId: "" },
    } satisfies PasteCredentialsState,
    review: { summary: [], confirmed: false } satisfies ReviewState,
    celebrate: { outroCopy, observatorySummary: null },
  };
}

function buildReviewSummary(states: StepStates) {
  const pasted = states["paste-credentials"] as PasteCredentialsState;
  const token = (pasted.values.accessToken ?? "").trim();
  const siteId = (pasted.values.siteId ?? "").trim();
  return [
    { label: "Access token", value: token ? maskWordPressToken(token) : "(not set)" },
    { label: "Site", value: siteId ? maskWordPressSiteId(siteId) : "(not set)" },
  ];
}

export function WordPressClient({
  audience,
  steps,
  outroCopy,
  expiryDays,
}: WordPressClientProps) {
  const {
    index,
    step,
    states,
    setStates,
    stepState,
    onStepStateChange,
    advance,
    handleCancel,
    onDone,
  } = useAdminShell({
    steps,
    initialStates: initialWordPressStates(outroCopy),
  });

  React.useEffect(() => {
    if (step.type !== "review-and-confirm") return;
    setStates((prev) => {
      const next = buildReviewSummary(prev);
      const current = (prev.review as { summary?: unknown[] } | undefined)
        ?.summary;
      if (
        Array.isArray(current) &&
        current.length === next.length &&
        current.every(
          (r, i) =>
            typeof r === "object" &&
            r !== null &&
            (r as { label?: string }).label === next[i].label &&
            (r as { value?: string }).value === next[i].value,
        )
      ) {
        return prev;
      }
      return {
        ...prev,
        review: { ...(prev.review as object), summary: next },
      };
    });
  }, [step.type, setStates]);

  const onComplete = React.useCallback(async (): Promise<CelebrationCompleteResult> => {
    const pasted = states["paste-credentials"] as PasteCredentialsState;
    const reviewed = states.review as ReviewState;
    const accessToken = pasted.values.accessToken.trim();
    const siteId = pasted.values.siteId.trim();
    const payload: WordPressPayload = {
      accessToken,
      siteId,
      verifiedAt: 0,
      confirmedAt: reviewed.confirmed ? Date.now() : 0,
    };
    return completeWordPressAction(payload);
  }, [states]);

  const configuredStep: WizardStepDefinition = React.useMemo(() => {
    if (step.type === "celebration") {
      return { ...step, config: { onDone, onComplete } };
    }
    if (step.type === "form") {
      return { ...step, config: { ...step.config, schema: wordpressCredentialsSchema } };
    }
    return step;
  }, [step, onComplete, onDone]);

  return (
    <WizardShell
      wizardKey="wordpress"
      currentStep={index}
      stepLabels={steps.map((s) => s.label)}
      audience={audience}
      expiryDays={expiryDays}
      onCancel={handleCancel}
      step={configuredStep}
      stepState={stepState}
      onStepStateChange={onStepStateChange}
      onNext={advance}
    />
  );
}
