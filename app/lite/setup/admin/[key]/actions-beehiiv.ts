...
function contractVersion(): string {
  const keys = beehiivWizard.completionContract.required
    .map((k) => String(k))
    .sort();
  return createHash("sha256")
    .update(JSON.stringify({ key: beehiivWizard.key, required: keys }))
    .digest("hex")
    .slice(0, 16);
}

export async function completeBeehiivAction(
  ...
  try {
    ...
    const { bandsRegistered } = await registerIntegration({
      wizardCompletionId,
      manifest: beehiivWizard.vendorManifest!,
      credentials: { plaintext: credentialBlob },
      metadata: { publication_id: payload.publicationId },
      ownerType: "admin",
      ownerId,
    });

    const verified = await verifyCompletion(beehiivWizard, payload, ctx);
    if (!verified.ok) {
