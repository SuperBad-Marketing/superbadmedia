...
import { runAuditEnrichment } from "@/lib/audit/enrichment";
import {
  scoreAuditCategory,
  scoreAuditOverall,
  AUDIT_CATEGORIES,
...
export async function POST(request: NextRequest) {
  ...
  // Generate per-category explanations
  ...
  // Generate per-category recommendations for the PDF
  const recommendations: Record<string, string> = {};
  for (const cs of categoryScores) {
    if (cs.available && explanations[cs.category]) {
  ...
  // Pipeline integration: create/reuse Deal
  ...
  // Update submission with pipeline linkage
  ...
  // Find weakest category for follow-up
  const weakest = categoryScores
    .filter((c) => c.available)
    .reduce((min, c) => (c.score < min.score ? c : min), categoryScores[0]);

  // Generate follow-up draft (async, non-blocking for response)
  const followUpPromise = generateAuditFollowUp({
    contactName: data.name,
    contactEmail: data.email,
  ...
  // Generate and deliver PDF report
  ...
  const deliveryPromise = deliverAuditReport({
    businessName: data.businessName,
    date: today,
    overall,
  ...
  // Wait for PDF + follow-up in parallel (these happen server-side, response goes out with scores)
  await Promise.allSettled([followUpPromise, deliveryPromise]);
